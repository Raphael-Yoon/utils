...
echo "=== [1/5] Docker Databases 기동 ==="
docker start snowball-mysql

echo "=== [2/5] AP Gunicorn 서버 재기동 ==="
...
echo "=== [3/5] Cloudflared 터널 프로세스 재기동 ==="
systemctl --user stop cloudflared-ksox cloudflared-trade cloudflared-infosd 2>/dev/null || true
...
echo "=== [4/5] Telegram Remote Bridge 재기동 ==="
pkill -f "telegram_bridge/bridge.py"
sleep 1
nohup "$WORKSPACE_DIR/.venv/bin/python" "$WORKSPACE_DIR/utils/telegram_bridge/bridge.py" > "$WORKSPACE_DIR/utils/telegram_bridge/bridge.log" 2>&1 &

echo "=== [5/5] 헬스체크 수행 및 알림 발송 ==="
sleep 3
